...
void grid::set_child(widget_ptr_t widget,
					  const unsigned row,
					  const unsigned col,
...
widget_ptr_t grid::swap_child(const std::string& id,
		widget_ptr_t w,
		const bool recurse,
		widget* new_parent)
{
	...
	for(auto & child : children_)
	{
		if(child.id() != id) {
			...
			if(recurse) {
				...
				if(g) {
					...
					widget_ptr_t old = g->swap_child(id, w, true);
					if(old) {
						return old;
					}
				}
		...
		widget_ptr_t old = child.free_widget();
		assert(old);
