...
std::shared_ptr<pane> pane::build(const implementation::builder_pane& builder)
{
	return std::shared_ptr<pane>(new pane(builder));
}
...
widget_ptr_t builder_pane::build() const
{
	return build(replacements_map());
}

widget_ptr_t builder_pane::build(const replacements_map& /*replacements*/) const
{
