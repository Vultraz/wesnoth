...
void scrollbar_container::layout_initialize(const bool full_initialization)
{
	...
	if(full_initialization) {
		...
		case AUTO_VISIBLE:
			vertical_scrollbar_grid_->set_visible(widget::visibility::hidden);
			break;
		...
		case AUTO_VISIBLE:
			horizontal_scrollbar_grid_->set_visible(widget::visibility::hidden);
			break;
...
void scrollbar_container::request_reduce_height(const unsigned maximum_height)
{
	...
	const unsigned offset =
		horizontal_scrollbar_grid_ && horizontal_scrollbar_grid_->get_visible() != widget::visibility::invisible
	...
	assert(vertical_scrollbar_grid_);
	const bool resized = vertical_scrollbar_grid_->get_visible() == widget::visibility::invisible;
...
void scrollbar_container::request_reduce_width(const unsigned maximum_width)
{
	...
	assert(content_grid_);
	const unsigned offset =
		vertical_scrollbar_grid_ && vertical_scrollbar_grid_->get_visible() != widget::visibility::invisible
	 ...
	 * minimum size.
	 */
	if(vertical_scrollbar_grid_->get_visible() != widget::visibility::invisible) {
...
point scrollbar_container::calculate_best_size() const
{
	...
	const point vertical_scrollbar = vertical_scrollbar_grid_->get_visible() == widget::visibility::invisible
		? point()
		: vertical_scrollbar_grid_->get_best_size();

	/***** get horizontal scrollbar size *****/
	const point horizontal_scrollbar = horizontal_scrollbar_grid_->get_visible() == widget::visibility::invisible
		? point()
...
static void set_scrollbar_mode(grid* scrollbar_grid,
...
static bool is_inserted_before(
		unsigned insertion_pos, unsigned old_item_count, unsigned old_position, unsigned visible_items)
{
	if(old_position == 0) {
		return false;
...
static void adjust_scrollbar_mode(grid* scrollbar_grid,
		scrollbar_base* scrollbar,
		scrollbar_container::scrollbar_mode& scrollbar_mode,
		...
		const unsigned visible_items)
{
	...
	const bool inserted_before_visible_area = is_inserted_before(
			static_cast<unsigned>(insertion_pos), items_before, previous_item_position, visible_items);
	...
	scrollbar->set_item_position(inserted_before_visible_area
		? previous_item_position + items_after - items_before
...
void scrollbar_container::place(const point& origin, const point& size)
{
	...
	const point content_grid_size(std::max(best_size.x, content_size.x), std::max(best_size.y, content_size.y));
...
widget* scrollbar_container::find_at(const point& coordinate, const bool must_be_active)
{
	widget* w = scrollbar_container_implementation::find_at<widget>(*this, coordinate, must_be_active);
	if(w == nullptr) {
...
const widget* scrollbar_container::find_at(const point& coordinate, const bool must_be_active) const
{
	const widget* w = scrollbar_container_implementation::find_at<const widget>(*this, coordinate, must_be_active);
	if(w == nullptr) {
		w = widget::find_at(coordinate, must_be_active);
	}
		...
		const int height_modification_pos)
{
	...
	const bool result =
		content_resize_width(width_modification, width_modification_pos) &&
...
bool scrollbar_container::content_resize_width(const int width_modification, const int width_modification_pos)
{
	...
	DBG_GUI_L << LOG_HEADER << " current width " << content_grid_->get_width() << " wanted width " << new_width;
...
bool scrollbar_container::content_resize_height(const int height_modification, const int height_modification_pos)
{
	...
	DBG_GUI_L << LOG_HEADER << " current height " << content_grid_->get_height() << " wanted height " << new_height;
...
void scrollbar_container::finalize_setup()
{
	...
	for(const auto& item : scroll_lookup()) {
		...
		if(button) {
			button->connect_click_handler(
				std::bind(&scrollbar_container::scroll_vertical_scrollbar, this, item.second));
		}
		...
		if(button) {
			button->connect_click_handler(
				std::bind(&scrollbar_container::scroll_horizontal_scrollbar, this, item.second));
		}
	}
	...
	content_grid_ = std::static_pointer_cast<grid>(get_grid().swap_child("_content_grid", content_, true));
	assert(content_grid_);
...
void scrollbar_container::set_vertical_scrollbar_mode(const scrollbar_mode scrollbar_mode)
{
...
void scrollbar_container::set_horizontal_scrollbar_mode(const scrollbar_mode scrollbar_mode)
{
...
void scrollbar_container::show_content_rect(const SDL_Rect& rect)
{
	...
	const int left_position = horizontal_scrollbar_->get_item_position() + (rect.x - content_->get_x());
	const int top_position = vertical_scrollbar_->get_item_position() + (rect.y - content_->get_y());
	...
	if(distance > 0) {
		vertical_scrollbar_->set_item_position(vertical_scrollbar_->get_item_position() + distance);
	}
	...
	if(distance > 0) {
		horizontal_scrollbar_->set_item_position(horizontal_scrollbar_->get_item_position() + distance);
	}

	// top.
	if(top_position < static_cast<int>(vertical_scrollbar_->get_item_position())) {
		vertical_scrollbar_->set_item_position(top_position);
	}

	// left.
	if(left_position < static_cast<int>(horizontal_scrollbar_->get_item_position())) {
...
void scrollbar_container::set_scrollbar_button_status()
{
	if(true) { /** @todo scrollbar visibility. */
		...
		vertical_scrollbar_->set_active(!vertical_scrollbar_->all_items_visible());
	}
	...
	if(true) { /** @todo scrollbar visibility. */
		...
		/***** Set the status if the scrollbars *****/
		horizontal_scrollbar_->set_active(!horizontal_scrollbar_->all_items_visible());
	}
}
...
void scrollbar_container::set_vertical_scrollbar_item_position(const unsigned position)
{
	assert(vertical_scrollbar_);
...
void scrollbar_container::set_horizontal_scrollbar_item_position(const unsigned position)
{
	assert(horizontal_scrollbar_);
...
void scrollbar_container::scroll_vertical_scrollbar(const scrollbar_base::scroll_mode scroll)
{
	assert(vertical_scrollbar_);
...
void scrollbar_container::scroll_horizontal_scrollbar(const scrollbar_base::scroll_mode scroll)
{
	assert(horizontal_scrollbar_);
...
void scrollbar_container::handle_key_page_up(SDL_Keymod /*modifier*/, bool& handled)
{
	assert(vertical_scrollbar_);
...
void scrollbar_container::handle_key_page_down(SDL_Keymod /*modifier*/, bool& handled)
...
void scrollbar_container::handle_key_up_arrow(SDL_Keymod /*modifier*/, bool& handled)
{
	assert(vertical_scrollbar_);
...
void scrollbar_container::handle_key_down_arrow(SDL_Keymod /*modifier*/, bool& handled)
{
	assert(vertical_scrollbar_);
...
void scrollbar_container::handle_key_left_arrow(SDL_Keymod /*modifier*/, bool& handled)
{
	assert(horizontal_scrollbar_);
...
void scrollbar_container::handle_key_right_arrow(SDL_Keymod /*modifier*/, bool& handled)
{
	assert(horizontal_scrollbar_);
...
void scrollbar_container::scrollbar_moved()
{
	...
	const int x_offset = horizontal_scrollbar_mode_ == ALWAYS_INVISIBLE
		? 0
		: horizontal_scrollbar_->get_item_position() * horizontal_scrollbar_->get_step_size();
